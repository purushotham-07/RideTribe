...
export default function GoogleLoginButton({
  ...
  if (variant === 'navbar') {
    return (
      <button
        ...
        disabled={loading}
        className={`group relative inline-flex items-center justify-center h-8 rounded-full border border-border bg-background hover:bg-secondary active:scale-95 transition-all text-foreground select-none cursor-pointer shrink-0 disabled:opacity-50 w-8 sm:w-auto sm:px-3 sm:py-1.5 ${className}`}
      ...
      >
        ...
        <span className="hidden sm:inline font-semibold text-xs text-foreground whitespace-nowrap ml-2">
  ...
  return (
    <button
      ...
      onClick={() => loginWithGoogle()}
      disabled={loading}
      className={`group relative inline-flex items-center justify-center bg-[#131314] hover:bg-[#202124] active:scale-[0.99] text-white border border-[#3c4043] rounded-2xl p-1.5 pl-2 pr-5 transition-all shadow-md hover:border-slate-400 disabled:opacity-50 select-none cursor-pointer whitespace-nowrap ${
        fullWidth ? 'w-full' : ''
      } ${className}`}
    ...
    >
      ...
      {/* Button Text */}
      <span className="font-bold text-sm text-white tracking-normal font-sans whitespace-nowrap">
